    ...
    public class ScoreManager
    {
        ...
        public void AddScore(string studentId, Score score)
        {
            ...
            if (string.IsNullOrWhiteSpace(studentId))
                throw new ArgumentException("学号不能为空");
            if (score == null)
                throw new ArgumentNullException(nameof(score), "成绩对象不能为空");
        ...
        public List<(string StudentId, double Average)> GetTopStudents(int count)
        {
            ...
            var averages = new List<(string, double Average)>();
                ...
                using (StreamReader reader = new(filePath))
                {
                    ...
                    while ((line = reader.ReadLine()) != null)
                    {
                        ...
                        if (parts.Length == 3)
                        {
                            ...
                            {
                                Console.WriteLine($"解析行错误: {line} - {ex.Message}");
                            }
                        }
                    }
                }
                Console.WriteLine($"从 {filePath} 加载了 {students.Count} 名学生");
            }
            catch (Exception ex)
            {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                {
                    var studentScores = scoreManager.GetStudentScores(student.StudentId);
                    double avg = scoreManager.CalculateAverage(student.StudentId);
                    Grade grade = scoreManager.GetGrade(avg);
                    
                    Console.WriteLine($"\n{student}");
                    Console.WriteLine($"  平均分: {avg:F1}, 等级: {grade}");
                    Console.WriteLine("  各科成绩:");
                    foreach (var score in studentScores)
                    {
                ...
                var studentInfo = studentManager.Find(s => s.StudentId == topStudent.StudentId).FirstOrDefault();
                Console.WriteLine($"{studentInfo?.Name} - 平均分: {topStudent.Average:F1}");
