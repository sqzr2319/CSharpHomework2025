...
namespace StudentManagementSystem
{
    ...
    public class StudentManager : IRepository<Student>
    {
        ...
        public void Add(Student item)
        {
            ...
            // 2. 添加到列表
            if (item == null)
                throw new ArgumentNullException(nameof(item), "Student to add cannot be null.");
    ...
    public class ScoreManager
    {
        ...
        {
            // TODO: 获取指定学生的所有成绩
            if (_scores.TryGetValue(studentId, out var scoresList))
            {
                return new List<Score>(scoresList); // Return a copy
            }
            return new List<Score>(); // Return empty list if not found
        }
        ...
        public double CalculateAverage(string studentId)
        {
            ...
            if (!_scores.ContainsKey(studentId) || _scores[studentId].Count == 0)
            {
                return 0.0;
            }
        ...
        public List<(string StudentId, double Average)> GetTopStudents(int count)
        {
            ...
            // Simple bubble sort-like logic to find top students
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                var foundStudents = studentManager.GetStudentsByAge(19, 20);
                ...
                if (topStudents.Any())
                {
                    var topStudentInfo = topStudents.First();
                    var studentDetails = studentManager.Find(s => s.StudentId == topStudentInfo.StudentId).First();
                    Console.WriteLine($"姓名: {studentDetails.Name}, 学号: {topStudentInfo.StudentId}, 平均分: {topStudentInfo.Average:F2}");
                }
                ...
                Console.WriteLine($"学生数据已保存至: {filePath}");
